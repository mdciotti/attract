...
function setMatrixUniforms() {
	...
	//mvMatrix = getRotationMatrix(toRad(rotx), toRad(roty), toRad(rotz));
	/*var mvMatrix = mat4.create();
	mat4.rotateX(mvMatrix, toRad(rotx), mvMatrix);
	mat4.rotateY(mvMatrix, toRad(roty), mvMatrix);
	mat4.rotateZ(mvMatrix, toRad(rotz), mvMatrix);*/
	gl.uniformMatrix4fv(mvUniform, false, mvMatrix);
	...
	//mat4.multiply(pMatrix, lMatrix, pMatrix);
	gl.uniformMatrix4fv(pUniform, false, pMatrix);
}
...
function initShaders() {
	...
	var vertices = new Float32Array(createAttractor(pointCount));
	gl.bufferData(gl.ARRAY_BUFFER, vertices, gl.STATIC_DRAW);
	gl.vertexAttribPointer(vattrib, 3, gl.FLOAT, false, 0, 0);
}

function createAttractor(amount) {
	var scale = 0.3,
		a = 2.5,
		b = 0.5,
		c = -0.9,
